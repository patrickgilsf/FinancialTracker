...
const getStockSpec = async () => {
  ...
  for (let stock of googleInputData) {
    ...
    if (stock.Symbol !== 'Symbol' && stock.Symbol) {
      ...
      //declare financial reports data variables
      let bv, bvps, bvps2, ni, cfo, totalAssets, sloan, qualityIndex;
      //if finanical reports 
      if (reportsQ.data[0]) { 
        bv = getReportData(reportsQ.data[0].report.bs, 'Total assets') - getReportData(reportsQ.data[0].report.bs, 'Total liabilities');
        bvps = financials.metric.bookValuePerShareQuarterly;
        bvps2 = bv / profile.shareOutstanding;
        console.log(bvps, bvps2);
        ni = getReportData(reportsQ.data[0].report.cf, 'Net income');
        cfo = getReportData(reportsQ.data[0].report.cf, 'Cash generated by operating activities');
        sloan = (ni - cfo) / totalAssets;
        totalAssets = getReportData(reports.data[0].report.bs, 'Total current assets');
        qualityIndex = sloan + roe + finalLev;
      }
